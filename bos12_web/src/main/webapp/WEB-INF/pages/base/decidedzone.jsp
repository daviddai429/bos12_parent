...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length == 1){
			...
			//页面加载完成
			//通过post请求加载左侧下拉框数据
			var url = "${pageContext.request.contextPath }/decidedzoneAction_findByNoRelation.action";
			$.post(url,{},function(data){
				//方法一：
				//var data2 = JSON.parse(data);
				$.each(data, function(i,n){
					$("#noassociationSelect").append("<option value='"+n.id+"'>"+n.name+"</option>");
				});
			})
			//方法二：
			//},"json")
			...
			//通过post请求加载右侧下拉框数据
			var url = "${pageContext.request.contextPath }/decidedzoneAction_findByHasRelation.action";
			$.post(url,{"id":rows[0].id},function(data){
				//var data = JSON.parse(data);
				$.each(data, function(i,n){
					$("#associationSelect").append("<option value='"+n.id+"'>"+n.name+"</option>");
				});
			})
			//},"json")
		}
		...
		{
			$.messager.alert('提示信息','请选择一行定区记录!','warning');  
		}
	}
	...
	function doDblClickRow(rowIndex, rowData){
		...
		$('#association_subarea').datagrid( {
			...
			//根据定区id请求分区模块的数据
			url : "${pageContext.request.contextPath}/subareaAction_findSubareaByDecidedzoneId.action?did="+id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${pageContext.request.contextPath}/decidedzoneAction_findByHasRelation.action?id="+id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
			...
			<div class="datagrid-toolbar">
				...
				<script type="text/javascript">
					$(function(){
						//页面加载完成 绑定保存事件
						$("#save").click(function(){
							var v = $("#saveDecidedzoneForm").form("validate");
							if(v){
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="saveDecidedzoneForm" action="${pageContext.request.contextPath }/decidedzoneAction_save.action" method="post">
							...
							<script type="text/javascript">
								$(function(){
									...
									$("#associationBtn").click(function(){
										...
										var did = $("#grid").datagrid("getSelections")[0].id;
										$("#customerDecidedZoneId").val(did);
										$("#customerForm").submit();
									})
								})
